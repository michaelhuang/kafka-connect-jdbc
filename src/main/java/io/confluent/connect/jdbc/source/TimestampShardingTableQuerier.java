 ...
 * <p>
 *     # job config
 *     mode: sharding+timestamp
 *     query: select * from tables:table_0,table_1
 *
 *     # build sql
 *     select * from
 *       (select * from table_0 WHERE  MODIFY_TIME  > ? AND  MODIFY_TIME  < ?
 *         union all
 *         select * from table_1 WHERE  MODIFY_TIME  > ? AND  MODIFY_TIME  < ? )
 *       ORDER BY MODIFY_TIME ASC
 * </p>
 */
public class TimestampShardingTableQuerier extends TableQuerier {
  private static final Logger log = LoggerFactory.getLogger(
      TimestampShardingTableQuerier.class
  );
  ...
  public TimestampShardingTableQuerier(QueryMode mode, String name, String topicPrefix,
  ...
  protected void createPreparedStatement(Connection db) throws SQLException {
    ...
    // sql格式: "select * from tables:table_0,table_1";
    switch (mode) {
      case QUERY:
    ...
    buildUnionAllSql(builder, quoteString);
    orderBy(builder, quoteString);
  ...
  private void buildUnionAllSql(StringBuilder builder, String quoteString) {
    String[] split = query.split("tables:");
    String[] tables = split[1].split(",");
    tablesCount = tables.length;
    int lastIndex = tablesCount - 1;
  ...
  protected ResultSet executeQuery() throws SQLException {
    if (incrementingColumn != null && timestampColumn != null) {
      ...
      for (int i = 0; i < tablesCount; i++) {
        stmt.setTimestamp(i * 4 + 1, endTime, DateTimeUtils.UTC_CALENDAR.get());
        stmt.setTimestamp(i * 4 + 2, tsOffset, DateTimeUtils.UTC_CALENDAR.get());
        stmt.setLong(i * 4 + 3, incOffset);
        stmt.setTimestamp(i * 4 + 4, tsOffset, DateTimeUtils.UTC_CALENDAR.get());
      }
      log.info(
      ...
      Timestamp endTime = new Timestamp(currentDbTime - timestampDelay);
      for (int i = 0; i < tablesCount; i++) {
        stmt.setTimestamp(i * 2 + 1, tsOffset, DateTimeUtils.UTC_CALENDAR.get());
        stmt.setTimestamp(i * 2 + 2, endTime, DateTimeUtils.UTC_CALENDAR.get());
      }
